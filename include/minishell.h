...
int			make_lst(t_token **token_list, t_token *token_node,
				int idx, int flag);
...
void		child_process(int *task, t_ast *node,
				int nb_pid, t_minishell *mini);
pid_t		fork_and_exec(int *task, t_ast *node,
				int nb_pid, t_minishell *mini);
...
int			handle_error(const char *file_content, int *svd_stdin,
				int *svd_stdout);
